...
public class DataReportService {
  ...
  public List<PostAndReactionsPerHourDto> getPostsAndReactionsPerHour() {
    ...
    postsList.forEach(
      perHour -> {
        var item = resultMap.get(perHour.getHour());
        item.setPosts(perHour.getAmount());
      }
    );
    ...
    reactionsList.forEach(
      perHour -> {
        var item = resultMap.get(perHour.getHour());
        item.setReactions(perHour.getAmount());
      }
    );

    var result = new ArrayList<PostAndReactionsPerHourDto>();
    resultMap.entrySet().stream().sorted(Map.Entry.comparingByKey()).forEach(element -> result.add(element.getValue()));
